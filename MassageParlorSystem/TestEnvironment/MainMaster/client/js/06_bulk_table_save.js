...
/**
 * 同步「全選」checkbox（桌機 + 手機）
 * 做法：
 * - 計算 filteredUsers 中被選到的數量
 * - 設定 checked/indeterminate
 */
function syncCheckAll_() {
  ...
  /**
   * @param {HTMLInputElement|null} el - checkbox
   * @param {boolean} checked - 是否全勾
   * @param {boolean} indeterminate - 是否半勾
   */
 ...
 * 批次刪除
 * 做法：
 * - 二次 confirm（含 dirty 警告）
 * - 逐筆呼叫 deleteUser + sleep
 * - 完成後 reload
 */
 ...
 * 做法：
 * - 清空 tbody
 * - 依 filteredUsers 逐列產生 tr.innerHTML
 */
...
/**
 * 處理列上欄位變更
 * @param {HTMLElement} fieldEl - 具有 data-field 的 input/select
 */
...
/**
 * 讀取 input/select 的值
 * @param {HTMLElement} el - input 或 select
 * @returns {string}
 */
...
/**
 * 刪除單一列
 * @param {HTMLTableRowElement} row - tr
 * @param {string} userId - userId
 * @param {HTMLButtonElement} delBtn - 刪除按鈕
 */
async function handleRowDelete_(row, userId, delBtn) {
  const u = allUsers.find((x) => x.userId === userId);
  const okConfirm = confirm(`確定要刪除使用者？\n\nuserId: ${userId}\n顯示名稱: ${u?.displayName || ""}\n\n此操作不可復原。`);
  if (!okConfirm) return;
 ...
 * 儲存所有 dirty 變更
 * 做法：
 * - 收集 dirtyIds -> 轉成 items
 * - 呼叫 updateUsersBatch
 * - 成功者更新 originalMap 並清掉 dirtyMap
 */
async function saveAllDirty_() {
  ...
  try {
    ...
    if (document.getElementById("footerStatus")) {
