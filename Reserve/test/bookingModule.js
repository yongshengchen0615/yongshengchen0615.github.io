export const BookingModule = (() => {
    ...
    function createPersonForm(index) {
        return `
            <div class="person-card shadow p-3 mb-3" data-person="${index}">
                <h5>預約人 ${index + 1}</h5>
                ${generateServiceSelectBlock("選擇主要服務", "main-service", mainServices, "btn-outline-primary", "main")}
                ${generateServiceSelectBlock("選擇加購服務", "addon-service", addonServices, "btn-outline-secondary", "addon")}
                <div class="mt-2">
    ...
    function generateServiceSelectBlock(title, selectClass, services, buttonClass, buttonType) {
        return `
            <label class="form-label mt-2">${title}</label>
            <div class="input-group">
                <select class="form-select ${selectClass}">
                    ${serviceOptionsGrouped(services)}
                </select>
                <button type="button" class="btn ${buttonClass} add-service" data-type="${buttonType}">確認</button>
            </div>
            <ul class="list-group ${selectClass}-list mt-2"></ul>
        `;
    }
