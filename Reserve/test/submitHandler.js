...
export function handleSubmit() {
    ...
    $form.on("submit", async function (event) {
        ...
        if (!validateName() || !validatePhone()) {
            NotificationModule.show("請確保姓名與手機格式正確！", "success");
            return;
        }
        ...
        if (!BookingTimeModule.isValidBookingTime($("#booking-date").val(), time)) {
            NotificationModule.show("當日預約已超過可預約時間，請選擇其他時段！", "success");
            return;
        }
...
💰 總金額：$${totalPrice} 元`;
        ...
        try {
            ...
            NotificationModule.confirm("✅ 預約確認訊息已成功傳送！").then(() => {
                BookingStorageModule.save(bookingData);
                liff.closeWindow();
            });
        } catch (err) {
            NotificationModule.show("⚠️ 發送訊息失敗：" + err, "success");
            console.error(err);
        }
    });
