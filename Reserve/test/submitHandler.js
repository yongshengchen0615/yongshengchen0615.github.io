...
export function handleSubmit() {
    ...
    $form.on("submit", async function (event) {
        ...
        const bookingDetails = bookingData.persons.map((p, i) => {
            const services = [...p.main, ...p.addon]
            .map(s => `${s.name}（${s.time} 分 / $${s.price}）`)
            .join("、");
        
          const timeSum = [...p.main, ...p.addon].reduce((sum, s) => sum + s.time, 0);
          const priceSum = [...p.main, ...p.addon].reduce((sum, s) => sum + s.price, 0);
