    ...
    function renderHistoryList(containerSelector) {
        ...
        history.forEach((record, index) => {
            ...
            const details = record.persons
                ...
                .join("<hr class=" + '"border-light"' + ">");
    
            const item = document.createElement("div");
            item.className = "accordion-item bg-dark text-white border-secondary";
    
            item.innerHTML = `
                <h2 class="accordion-header" id="heading-${index}">
                    <button class="accordion-button collapsed bg-secondary text-white" type="button" data-bs-toggle="collapse" data-bs-target="#${collapseId}" aria-expanded="false" aria-controls="${collapseId}">
                        ${summary} ｜💰 $${totalPrice} 元
                    </button>
                </h2>
                <div id="${collapseId}" class="accordion-collapse collapse" aria-labelledby="heading-${index}" data-bs-parent="#${accordionId}">
                    <div class="accordion-body bg-dark text-white">
                        ...
                        <div class="d-flex gap-2 justify-content-end">
                            <button class="btn btn-sm btn-outline-light restore-btn">還原</button>
                            <button class="btn btn-sm btn-outline-light delete-btn">刪除</button>
                        </div>
                    </div>
                </div>
            `;
