...
$(document).ready(async function () {
    try {
        ...
        // 🛑 不強制登入，允許未登入的使用者使用
        if (!liff.isInClient()) {
            NotificationModule.show("⚠️ 注意：目前不在 LINE 應用內，功能可能無法使用。", "error");
        }
        // 獲取用戶資訊
        ...
        NotificationModule.show("無法載入 LIFF，請重新整理頁面！", "success");
    }
    ...
    $("#clear-history").click(() => {
        ...
        NotificationModule.show("✅ 已清除上次預約紀錄！", "success");
    });
    ...
    $("#clear-all-storage").click(() => {
        if (confirm("⚠️ 確定要清除所有儲存資料嗎？這個動作無法還原。")) {
            localStorage.clear();
            NotificationModule.show("✅ 已清除所有儲存資料！", "success");
