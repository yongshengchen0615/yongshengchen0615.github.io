    ...
    .then(() => {
        ...
        $(".person-card").each(function () {
            ...
            $(this).find(".main-service-list li").each(function () {
                mainServices.push($(this).clone().children("button").remove().end().text().trim());
            });
            $(this).find(".addon-service-list li").each(function () {
                addonServices.push($(this).clone().children("button").remove().end().text().trim());
            });
        
            bookingData.people.push({ mainServices, addonServices });
        });
        
        // 儲存到 localStorage（混合格式）
...
HistoryModule.renderHistory("#history-container", (data) => {
    BookingModule.setFormFromHistory(data);
});
