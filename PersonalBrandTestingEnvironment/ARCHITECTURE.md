...
| `LotteryTypes` | 管理 GAS | 轉盤類型生命週期與抽獎券獎項預覽開關 |
...
- 抽獎券摘要沿用 `upsertMember` 已算出的集點卡狀態；啟用獎項預覽時只附上最新轉盤的獎項名稱，不傳機率。開啟滿版票券清單不再額外讀取試算表；選定可用券後，首頁轉盤小視窗會在載入狀態中取得最新設定並以固定 request ID 安全準備結果，中央按鈕只負責動畫，不再等待網路。
...
- 保留頁面、API action 與 GAS 部署方式；`LotteryTypes` 只在尾端追加相容欄位，舊列由 `setup()` 補成關閉預覽。
