...
test("member-lottery-v2 exposes the existing controller without a prepared-draw facade", () => {
  ...
  const window = {
    ...
    document: {},
    MemberApi: { createRequestId() { return "runtime-request-0001"; } },
    LotteryWheel: {},
  };
  window.window = window;
  ...
  vm.runInContext(
    bootstrapSource,
    vm.createContext({ window, Error, Object, Promise })
  );

  assert.equal(window.MemberLotteryDialog, inner);
...
test("composition root cannot consume a ticket or cache an unrevealed prize", () => {
  assert.doesNotMatch(bootstrapSource, /["']drawLottery["']/);
  assert.doesNotMatch(bootstrapSource, /persona-member-lottery-prepared:/);
  assert.doesNotMatch(bootstrapSource, /preparedEntries|predraw|buildVirtualCard/);
