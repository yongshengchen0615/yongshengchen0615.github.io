(function () {
  ...
  var INVALID_TOKEN_RECOVERY_PREFIX =
    "persona-member-lottery-token-recovery:";
  ...
  function boot() {
    ...
    wheelPreparationVersion += 1;

    stopSpinAnimation();

    byId("lottery-state").setAttribute(
      "aria-busy",
      "false"
    );
  ...
  function renderWorkspace(data) {
    lotteryTypes = normalizeLotteryTypes(
      data.lotteryTypes
    );

    cardStatus = normalizePointCardStatus(
      data.card
    );
    ...
    if (
      ...
      selectedRewardTicket =
        cardStatus.availableRewards.find(
          function (ticket) {
            return (
              ticket.cardRoundKey ===
              requestedCardRoundKey
      ...
      if (selectedRewardTicket) {
        selectedLotteryTypeId =
          selectedRewardTicket.lotteryTypeId;
      } else {
        requestedTicketError =
          "這張抽獎券已使用或不存在，請返回會員資料重新選擇。";
      }
    }
  ...
  function normalizePointCardStatus(
    value
  ) {
    value =
      value &&
    ...
    var rewardMilestones =
      Array.isArray(
        ...
        ? value.rewardMilestones.map(
            Number
          )
        : [];

    var rewardRules =
      Array.isArray(value.rewardRules)
        ? value.rewardRules.map(
            function (rule) {
              rule =
                rule &&
                typeof rule === "object"
    ...
    var availableRewards =
      Array.isArray(
        ...
        ? value.availableRewards.map(
            normalizeRewardTicket
          )
        : [];

    var normalized = {
      settingVersion: String(
        value.settingVersion || ""
      ).trim(),
      targetPoints: Number(
        value.targetPoints
      ),
      ...
      nextMilestonePoints: Number(
        value.nextMilestonePoints
      ),
      pointsRemaining: Number(
        value.pointsRemaining
      ),
      pointsToCardComplete: Number(
        value.pointsToCardComplete
      ),
    ...
    if (
      ...
      (
        normalized.expiryMode ===
          "unlimited" &&
        normalized.expiresOn
      ) ||
      (
        normalized.expiryMode ===
          "limited" &&
        !isValidPointCardDate(
          normalized.expiresOn
        )
      ) ||
      ...
      normalized.rewardRules.length !==
        normalized.rewardMilestones
          .length ||
      normalized.rewardRules.some(
        function (rule, index) {
          return (
            rule.points !==
              normalized
      ...
      normalized.reachedMilestones.some(
        function (milestone) {
          return (
            normalized.rewardMilestones.indexOf(
              milestone
            ) === -1
          );
        }
      ...
      !pointSequencesEqual(
        normalized.reachedMilestones,
        normalized.rewardMilestones.filter(
          function (milestone) {
            return (
              milestone <=
              normalized.currentPoints
      ...
      normalized.nextMilestonePoints !==
        normalized.rewardMilestones.find(
          function (milestone) {
            return (
              milestone >
              normalized.currentPoints
      ...
      normalized.currentRound !==
        normalized.currentCardNumber ||
      !Number.isInteger(
        normalized.completedCards
      ) ||
      normalized.completedCards < 0 ||
      !Number.isInteger(
        normalized.completedRounds
      ) ||
      normalized.completedRounds !==
        normalized.completedCards ||
      !Number.isInteger(
        normalized.earnedRewards
      ) ||
      normalized.earnedRewards < 0 ||
      !Number.isInteger(
        normalized.drawsUsed
      ) ||
      ...
      normalized.availableDraws >
        normalized.earnedRewards -
          normalized.drawsUsed ||
      normalized.availableRewards
        .length !==
        Math.min(
          normalized.availableDraws,
          50
        ) ||
      !hasUniqueRewardTickets(
        normalized.availableRewards
      ) ||
      normalized.availableRewards.some(
        function (ticket) {
  ...
  function hasUniqueRewardTickets(
    tickets
  ) {
  ...
  function isStrictPointSequence(
    values,
    maximum,
    allowEmpty
  ) {
  ...
  function renderPointCard() {
    ...
    byId(
      ...
      cardStatus.expiryMode ===
      "limited"
        ? "有效至 " +
          cardStatus.expiresOn.replace(
            /-/g,
            "."
          )
        : "集點卡無期限";
    ...
    track.setAttribute(
      "aria-valuemax",
      String(cardStatus.targetPoints)
    );

    track.setAttribute(
      "aria-valuenow",
      String(cardStatus.currentPoints)
    );
  ...
  function renderLotteryTickets() {
    ...
    var lockedContainer = byId(
      "locked-ticket-list"
    );

    var lockedEmpty = byId(
      "locked-ticket-empty"
    );
    ...
    cardStatus.availableRewards.forEach(
      function (ticket, index) {
        var type = findLotteryType(
          ticket.lotteryTypeId
        );
    ...
    lockedContainer.replaceChildren(
      lockedFragment
    );
  ...
  function handleTicketTabKeydown(
    event
  ) {
    if (
      event.key !== "ArrowLeft" &&
  ...
  function openLotteryTicket(ticket) {
    ...
    prepareLotteryTicket(
      ...
      .then(function () {
        ...
        byId(
          "lottery-spin-status"
        ).textContent = pendingRequest
          ? "上次結果尚未確認，點選中央安全重試。"
          : "轉盤已就緒，點選中央開始抽獎。";

        updateControls();
        focusLotterySpinButton();
      })
      .catch(function (error) {
        if (
          preparationVersion !==
  ...
  function prepareLotteryTicket(
    ticket,
    expectedPreparationVersion
  ) {
    if (isDemoSession) {
      renderSelectedLottery();
      return waitForWheelPaint();
    }
    ...
    return sendMemberRequest(
      ...
      var refreshedTicket = null;

      if (
        storedRequest &&
        storedRequest.cardRoundKey ===
          ticket.cardRoundKey &&
        storedRequest.lotteryTypeId ===
          ticket.lotteryTypeId
      ) {
        refreshedTicket =
          pendingTicketResponse(
      ...
      cardStatus = refreshedCard;
      selectedRewardTicket =
        refreshedTicket;
      selectedLotteryTypeId =
        refreshedTicket.lotteryTypeId;
  ...
  function renderSelectedLotteryHeading() {
    if (!selectedRewardTicket) return;
    ...
    byId(
      ...
      "第 " +
      formatNumber(
        selectedRewardTicket.cardNumber
      ) +
      " 張集點卡 · " +
      formatNumber(
        selectedRewardTicket
  ...
  function renderSelectedLottery() {
    ...
    drawWheel(
      selected.lottery.prizes,
      selected.lotteryTypeId
    );

    resetRotor();
  }

  function waitForWheelPaint() {
    return new Promise(
      function (resolve) {
  ...
  function focusLotterySpinButton() {
    window.requestAnimationFrame(
      function () {
        try {
          byId(
  ...
  function handleDraw() {
    ...
    if (isDemoSession) {
      var previewPrize =
        selected.lottery.prizes[1] ||
        selected.lottery.prizes[0];
      ...
      window.setTimeout(
        function () {
          finishDraw({
            ...
            draw: {
              drawId:
                "LDW-PREVIEW000000001",
              configVersion:
                selected.lottery
                  .configVersion,
              prizeId:
                previewPrize.prizeId,
              prizeLabel:
                previewPrize.label,
              prizeColor:
                previewPrize.color,
            ...
            card: {
              settingVersion:
                cardStatus.settingVersion,
              targetPoints:
                cardStatus.targetPoints,
              expiryMode:
                cardStatus.expiryMode,
              expiresOn:
                cardStatus.expiresOn,
              rewardMilestones:
                cardStatus.rewardMilestones.slice(),
              rewardRules:
                cardStatus.rewardRules.slice(),
              reachedMilestones:
                cardStatus.reachedMilestones.slice(),
              currentPoints:
                cardStatus.currentPoints,
              nextMilestonePoints:
                cardStatus.nextMilestonePoints,
              ...
              pointsToCardComplete:
                cardStatus.pointsToCardComplete,
              currentCardNumber:
                cardStatus.currentCardNumber,
              currentRound:
                cardStatus.currentRound,
              completedCards:
                cardStatus.completedCards,
              completedRounds:
                cardStatus.completedRounds,
              earnedRewards:
                cardStatus.earnedRewards,
              drawsUsed:
                cardStatus.drawsUsed +
                1,
              availableDraws:
                cardStatus
                  .availableDraws - 1,
              availableRewards:
                cardStatus.availableRewards.filter(
                  function (ticket) {
                    return (
                      ticket.cardRoundKey !==
                      selectedRewardTicket
  ...
  function normalizeDraw(
    ...
    var draw = {
      ...
      pointsSpent: Number(
        value.pointsSpent == null
          ? value.ticketCost
          : value.pointsSpent
      ),
      originalPointBalance: Number(
        value.originalPointBalance
      ),
      pointBalance: Number(
        value.pointBalance
      ),
    ...
    var prize =
      selectedType.lottery.prizes.find(
        function (item) {
          return (
            item.prizeId ===
            draw.prizeId
    ...
    if (
      ...
      draw.pointBalance !==
        draw.originalPointBalance ||
      !/^PCS-[A-Z0-9]{12}:[1-9]\d*(?::[1-9]\d*)?$/.test(
        draw.cardRoundKey
      ) ||
      Number.isNaN(
        new Date(
  ...
  function animateToPrize(
    ...
    var animationVersion =
      spinAnimationVersion;

    var prizeIndex =
      lottery.prizes.findIndex(
        function (prize) {
          return (
            prize.prizeId ===
            draw.prizeId
  ...
  function setMemberRoutesLocked(
    locked
  ) {
    document
      .querySelectorAll(
  ...
  function preventMemberRouteDuringSpin(
    event
  ) {
    var link =
      event.target &&
  ...
  function preventPageExitDuringSpin(
    event
  ) {
    if (!isBusy) return;
  ...
  function ensurePendingRequest(
    ...
    var request = {
      requestId:
        window.MemberApi.createRequestId(),
      settingVersion:
        ticket.settingVersion,
      cardNumber:
        ticket.cardNumber,
    ...
    try {
      ...
      // sessionStorage 不可用時，仍保留記憶體內的 request。
    }
  ...
  function readPendingRequest() {
    ...
    try {
      ...
      if (
        parsed &&
        /^[a-zA-Z0-9-]{10,80}$/.test(
          parsed.requestId
        ) &&
        isValidPendingTicket(
          parsed
        )
      ) {
      ...
      // 無效或不可用的 storage 視為空資料。
    }

    return null;
  }

  function isValidPendingTicket(
    value
  ) {
    try {
  ...
  function isRestorablePendingTicket(
    value
  ) {
    return (
  ...
  function pendingTicketResponse(
    value
  ) {
    return normalizeRewardTicket(
      value
    );
  }
  ...
  function normalizeError(
    ...
    var messages = {
      INVALID_TOKEN:
        "LINE 登入憑證無效或已過期，請重新登入。",
      INVALID_ID_TOKEN:
        "LINE 登入憑證已失效，請重新登入。",
      MISSING_ID_TOKEN:
        "沒有取得 LINE 登入憑證，請確認 LIFF openid 權限。",
      MEMBER_NOT_FOUND:
        "請先返回會員資料頁完成會員登入。",
      ...
      LOTTERY_DATA_ERROR:
        "抽獎紀錄目前無法使用，請聯絡管理員。",
      REQUEST_ID_CONFLICT:
        "上一次抽獎仍在確認中，請使用同一轉盤重試。",
      ORIGIN_NOT_ALLOWED:
        "目前網站來源未被 GAS 允許。",
      LINE_RATE_LIMITED:
        "LINE 驗證請求較多，請稍候一分鐘再試。",
      LINE_UNAVAILABLE:
        "LINE 驗證服務暫時無法使用。",
      BUSY:
        "抽獎資料正在更新，請稍候幾秒再試。",
  ...
  function getLiffContext() {
    return window.LiffRuntime.getContext(
      window.liff,
      window.navigator
    );
  }
  ...
  function hasCompleteConfig() {
    return window.LiffRuntime.hasCompleteConfig(
      CONFIG,
      window.MemberApi
    );
  }
  ...
  function hasDemoQuery() {
    return window.LiffRuntime.hasDemoQuery(
  ...
  function bindInteractions() {
    document.addEventListener(
      "click",
      preventMemberRouteDuringSpin,
      true
    );

    window.addEventListener(
      "beforeunload",
      preventPageExitDuringSpin
    );
