...
- 管理員可新增或軟刪除轉盤類型，各類型可設定 2～12 個獎項的名稱、色塊與中獎機率。會員在獨立抽獎頁選擇其中一種轉盤，每個已取得的節點資格只能抽一次。
...
電話與生日會追加為 `Members.phone` 與 `Members.birthday`，原有 1–21 欄位置不會改變。點數功能使用 `PointTypes`、`PointCampaigns`、`PointRedemptions`；集點卡與轉盤功能使用 `PointCardSettings`、`LotteryTypes`、`LotteryPrizes`、`LotteryDraws`，都不會增加 `Members` 欄位。新版 `setup()` 會替舊點數資料補上期限／領取規則快照、在 `PointCardSettings` 尾端追加 `reward_milestones`、建立預設 5 點集點卡與「經典轉盤」，並把既有轉盤獎項歸入該預設類型。舊集點卡規則的節點欄會保持空白並相容解讀為「只在滿點抽一次」；既有活動預設遷移為「有期限＋每位會員一次」。升級時請在維護時段連續完成：
