...
- 會員可使用固定 5 點取得一張轉盤抽獎券；GAS 會先檢查餘額、扣點並決定獎項，前端只依後台結果播放動畫。管理員可設定 2～12 個獎項的名稱、色塊與中獎機率，機率合計必須是 100%。
...
5. 執行一次 `setup()` 並完成授權，建立或驗證 `Members`、三張點數工作表及 `LotteryPrizes`、`LotteryDraws`。
...
5. 執行一次 `setup()` 並完成授權，建立或驗證七張工作表。`setup()` 也會自動建立管理 GAS 專用的 `POINT_CLAIM_SECRET`；不要放進前端 JSON、試算表或公開文件。
...
電話與生日會追加為 `Members.phone` 與 `Members.birthday`，原有 1–21 欄位置不會改變。點數功能使用 `PointTypes`、`PointCampaigns`、`PointRedemptions`，轉盤功能另建立 `LotteryPrizes` 與 `LotteryDraws`，都不會增加 `Members` 欄位。新版 `setup()` 會替舊點數資料補上期限／領取規則快照，並建立兩張轉盤工作表；既有活動預設遷移為「有期限＋每位會員一次」。升級時請在維護時段連續完成：

1. 同時將最新的會員 `Code.gs` 與管理 `Code.gs` 貼入各自專案。
2. 先在管理 GAS、再在會員 GAS 各執行一次 `setup()`，確認 `Members` 為 23 欄、三張點數工作表已完成升級，並已建立兩張轉盤工作表。
...
5. 會員中心會透過 `listPointHistory` 合併本人最近 30 筆領點與轉盤扣點紀錄；其他會員的 LINE User ID、Email 與內部欄位不會送到前端。
6. 管理員點數頁會透過 `adminListPointHistory` 顯示共用 `PointRedemptions` 最近 50 筆領取紀錄，包含會員編號、增加點數、領取後餘額與領取規則；LINE User ID、請求識別碼等內部欄位不會送到前端。
...
QR 連結可以被轉傳，因此它代表「持有連結即可參加」而不是現場定位或一次性票券。尤其「無期限＋可重複」QR 外流後，任何仍具會員資格的人都能反覆領點，請只在能接受此風險時使用。若要提前停止活動，可由試算表擁有者把對應 `PointCampaigns.status` 改為 `inactive`。刪除會員資料時也會刪除該會員的 `PointRedemptions` 與 `LotteryDraws`，因此點數餘額、領取及抽獎歷史會一併清除；若日後重新建立會員，仍有效的舊 QR 可依活動規則重新領取。

## 5 點轉盤抽獎流程

1. 管理員前往 `admin/lottery.html`，設定 2～12 個獎項的名稱、`#RRGGBB` 色塊與中獎機率。每個機率最多兩位小數，合計必須精確等於 100%。
2. 儲存時管理 GAS 會在 `LotteryPrizes` 追加完整的新版本；舊版本不覆寫，讓既有抽獎紀錄仍可核對當時的獎項快照。
3. 會員在會員中心按「使用 5 點轉盤抽獎」。會員 GAS 會重新確認會員可使用、目前餘額至少 5 點，再由後台依設定機率決定獎項並在 `LotteryDraws` 寫入扣點前後餘額。
4. 前端不自行抽獎，只依 GAS 回傳的獎項旋轉到對應色塊。完成後顯示獎項、原本點數、使用 5 點與目前點數；按「確認」才回到會員資料。
5. 每次操作會保留固定 request ID。網路逾時後重試會重播同一筆結果，不會再次扣 5 點。
6. 管理端轉盤頁顯示最新 50 筆抽獎紀錄，但不回傳 LINE User ID 或 request ID。
...
會員 LIFF ID Token
  ...
  → LotteryPrizes：讀取目前轉盤版本與機率
  → LotteryDraws：後台決定獎項後寫入 5 點扣點與結果快照
...
管理 LIFF ID Token
  ...
  → status=approved 後才可追加 LotteryPrizes 設定版本與讀取 LotteryDraws
```
