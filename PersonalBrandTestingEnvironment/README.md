...
- 已核准會員掃描 QR 並登入後會直接領取點數；完成畫面按「確認」即可關閉，活動可設定每會員一次或重新掃描後重複領取。
...
1. 管理員切換到 `admin/points.html` 點數管理頁，輸入 `1`～`9999` 的整數點數，並選擇「有期限／無期限」與「不可重複／可重複」後新增類型。同一點數可依規則組合建立多種類型。
2. 選取類型產生 QR Code。有期限類型須設定未來到期時間（最長 366 天）；無期限類型不顯示日期欄，可直接產生 QR。
3. 會員掃描後登入，系統先驗證會員仍為 `approved`、活動為 `active`，有期限活動還會核對是否到期。
4. 會員掃描並完成登入後，會員 GAS 直接把領取紀錄寫入 `PointRedemptions`；完成畫面按「確認」關閉，並從領取紀錄加總目前餘額。
5. 管理員可刪除不再使用的點數類型。刪除採停用方式保存稽核資料，只阻止產生新的 QR；既有 QR 仍依建立當時的規則快照運作。
