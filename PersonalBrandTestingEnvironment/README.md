...
- 會員領點成功後，會員端會使用 LIFF `sendMessages()` 以會員身分把「獲得點數／目前餘額」通知送到官方帳號的一對一聊天室；若頁面不是從該聊天室開啟，點數仍會完成發放，但會顯示無法自動傳訊息的原因。
...
另外，會員端 LIFF App 必須勾選 `chat_message.write`，用來讓會員透過 LIFF `sendMessages()` 將領點通知傳給官方帳號；管理端不需要這個 Scope。

會員端 LIFF Channel 還必須在 LINE Developers Console 連結會員官方帳號，且該 LIFF 頁面要從官方帳號的一對一聊天室開啟，`sendMessages()` 才會把訊息送到正確的官方帳號。若會員尚未加入官方帳號，系統會先開啟 `client/config.json` 的 `OFFICIAL_ACCOUNT_FRIEND_URL`，未完成加入就不會寫入點數；LINE 不允許網頁靜默替使用者加好友。從一般瀏覽器或外部 QR 頁面開啟時，也會先導向官方帳號加入網址。
...
- [LIFF API：好友狀態與 `sendMessages()`](https://developers.line.biz/en/reference/liff)
- [將 LINE Login Channel 連結官方帳號](https://developers.line.biz/en/docs/line-login/link-a-bot/)
...
```json
{
  ...
  "OFFICIAL_ACCOUNT_FRIEND_URL": "https://lin.ee/vdtjCdT"
}
```
