...
電話與生日會追加為 `Members.phone` 與 `Members.birthday`，原有 1–21 欄位置不會改變。升級時請在維護時段連續完成：

1. 同時將最新的會員 `Code.gs` 與管理 `Code.gs` 貼入各自專案。
2. 兩個 GAS 都建立新的正式部署版本。
3. 在兩邊各執行一次 `setup()`，確認 `Members` 為 23 欄。
4. 最後再發布前端。
