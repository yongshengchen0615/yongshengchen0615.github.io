...
1. 初始狀態沒有轉盤。管理員前往 `admin/lottery.html` 按「新增第一個轉盤」，在同一個表單設定名稱、是否在抽獎券顯示可中獎項、2～12 個獎項、`#RRGGBB` 色塊與中獎機率，預覽確認後只按一次「儲存並啟用轉盤」。每個機率最多兩位小數，合計必須精確等於 100%；抽獎券預覽只顯示獎項名稱，不公開機率。
...
4. 會員卡以一點一顆星呈現本輪集點，不使用進度條；可取得抽獎券的節點改用星芒標示。「抽獎券」會開啟滿版清單並切換「未獲得／已獲得」，啟用獎項預覽的轉盤會在兩種票券下方列出可抽中的獎項；「點數紀錄」也使用相同的滿版高度與獨立內容捲動。點選可用券後，會員首頁會立即顯示「正在準備轉盤」，獎項、圖面與本次結果都安全準備完成後才切換為置中的小型轉盤；`client/lottery.html` 僅保留舊連結相容。
