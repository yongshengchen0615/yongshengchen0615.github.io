...
會員端 LIFF Channel 還必須在 LINE Developers Console 連結會員官方帳號，且該 LIFF 頁面要從官方帳號的一對一聊天室開啟，`sendMessages()` 才會把訊息送到正確的官方帳號。從 LINE LIFF 領點時若會員尚未加入官方帳號，系統會先呼叫 `requestFriendship()` 顯示加入確認，未完成加入就不會寫入點數；LINE 不允許網頁靜默替使用者加好友。從一般瀏覽器或外部 QR 頁面開啟時，系統仍會發放點數，但不會把訊息誤送到其他聊天室。
