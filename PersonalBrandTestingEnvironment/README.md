...
如果還不知道自己的 LINE User ID，可先不設定 `ADMIN_LINE_USER_IDS`：

1. 先從會員端 LIFF 登入，讓系統以 `pending` 建立你的會員列。
2. 到 Members 工作表複製該列的 `line_user_id`。
3. 回到 GAS 指令碼屬性，設定 `ADMIN_LINE_USER_IDS=U...`；多位管理員以半形逗號分隔。
4. 從管理端 LIFF 登入，核准自己的會員帳號。

管理員名單只存在 GAS Script Properties。`client/` 與 `admin/` 在 GitHub Pages 上仍是相同 origin，資料夾或網址本身不構成管理權限。
...
以 LINE 回傳的 sub 當唯一會員鍵並更新 Google Sheet
        ↓
新會員為 pending；只有 approved 才回傳會員中心資料
```
...
- `pending`／`approved`／`denied` 審核狀態、建立／更新／最後登入時間，以及依不同 LINE ID Token 工作階段計算的登入次數
...
管理端每次讀取或修改會員資料時，都會先向 LINE 驗證管理員的 ID Token，再以驗證後的 `sub` 精確比對 GAS 私有的 `ADMIN_LINE_USER_IDS`。管理端網址、前端 JSON、`callbackOrigin` 或前端傳入的角色都不會被當成管理權限。新會員登入只會更新自己的 LINE 個資與登入紀錄，不會覆寫管理員設定的審核狀態；待審核或已拒絕會員仍可刪除自己的資料。
...
### 管理端顯示 `ADMIN CONFIG ERROR`

先從會員端登入一次，在 Members 工作表找到自己的 `line_user_id`，再把完整值填入 GAS Script Property `ADMIN_LINE_USER_IDS`。不要填會員編號 `MBR-...`。

### 管理端顯示「沒有管理權限」

目前 LINE 帳號的驗證後 User ID 不在 `ADMIN_LINE_USER_IDS`。確認沒有多餘空白、漏字，且多個 ID 使用半形逗號分隔。
