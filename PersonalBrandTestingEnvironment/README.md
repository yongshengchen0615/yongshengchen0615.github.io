...
- 管理員可新增或軟刪除轉盤。新增時在同一個編輯區完成名稱、2～12 個獎項、色塊與中獎機率，最後只需按一次「儲存並啟用轉盤」。會員在獨立集點卡頁查看抽獎券紀錄，轉盤資料會先預載；點選可用券後才開啟對應轉盤，並直接點轉盤中央開始抽獎。抽完的券會保留並標示為「已使用」。
...
5. 抽獎不扣點。會員 GAS 會在鎖定狀態下重新確認會員權限、抽獎券與指定轉盤，由後台依設定機率決定獎項，再把轉盤類型、設定版本、集點卡規則版本、卡片編號與節點寫入 `LotteryDraws`。抽獎前後累計點數相同；結果確認後回到集點卡，已抽過的券保留並標示為「已使用」。
