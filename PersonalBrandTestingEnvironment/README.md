...
- 開啟轉盤時只呼叫一次 `prepareLotteryDraw`，後台會同時驗證抽獎券、保存結果並回傳最新轉盤；中央按鈕只播放 5 秒由快到慢的動畫，不會在旋轉途中讀取網路。
- 裝置會依每個 GAS URL 記住最近成功的傳輸方式。Fetch 最多等待 9 秒；只有網路層失敗時才改用受驗證的 iframe bridge，bridge 最多等待 25 秒，以容納 GAS 冷啟動。只儲存 `fetch`／`bridge` 名稱，不儲存會員資料；會員資料與集點卡內容仍不做瀏覽器快取，每次以 GAS 與 Spreadsheet 最新資料為準。
...
3. `PointRedemptions` 的點數永遠代表終身累計，會員卡會把這個數字放在會員名稱旁並顯示為「目前消費時數」。以上述 20 點卡為例，本張卡到 5、10、15、20 點時各產生一張指定轉盤的抽獎券；到 20 點後會員畫面仍顯示第 2 張卡「目前已集 0 點／本輪目標 20 點」，目前消費時數則保留為 20。
