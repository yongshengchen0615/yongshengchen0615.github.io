...
- 同一分頁會記住成功的 GAS 傳輸方式。Fetch 最多等待 9 秒，iframe bridge 最多等待 12 秒；會員資料與集點卡內容不做瀏覽器快取，每次仍以 GAS 與 Spreadsheet 最新資料為準。
- 會員 GAS 對同一個有效 LINE ID Token 最多快取 5 分鐘的 `sub`、`iat`、`exp` 驗證結果；原始 Token、姓名與頭像不會進入快取。只有逾時、失敗或超過目標時間的匿名效能事件會送到 Cloud Logging。
...
新版會員前端需要會員 GAS `1.13.0` 的 `prepareLotteryDraw` 與 `reportClientPerformance` action。發布時必須先更新並部署會員 GAS，確認健康檢查版本後，再發布 `shared/` 與 `client/`；不要先發布新版前端，否則抽獎準備會收到 `UNSUPPORTED_ACTION`。
