...
- 會員不需要加入官方帳號即可集點；會員頁提供「掃描集點 QR Code」按鈕。若頁面是在官方帳號一對一聊天室中開啟，且 LIFF 具備 `chat_message.write` 權限，才會額外嘗試用 `sendMessages()` 傳送領點通知，通知失敗不影響點數發放。
...
會員端若要在官方帳號一對一聊天室中嘗試傳送領點通知，可另外勾選 `chat_message.write`；這個權限不是集點必要條件，管理端不需要這個 Scope。會員不需要加入官方帳號，也可以登入、掃描 QR 與完成領點。

會員端 LIFF App 必須在 LINE Developers Console 開啟 `Scan QR` 功能，會員頁的「掃描集點 QR Code」按鈕才會呼叫 `liff.scanCodeV2()`。建議 Size 選 `Full`，並在實機上授予相機權限。從管理端產生的點數 QR 也可以直接開啟會員 LIFF，保留原本的自動領點流程。
...
到 LINE Developers Console 的會員 LIFF 設定開啟 `Scan QR`，並將 Size 設為 `Full`；儲存後關閉目前 LIFF 視窗，再從會員 LIFF URL 重新開啟。若仍無法使用，請更新 LINE App，或改用支援 WebRTC 的手機瀏覽器；LINE 官方也要求 `scanCodeV2()` 只能在支援的 LIFF／瀏覽器環境執行。
