...
- 管理員可設定每輪集點門檻。會員達標後，集點卡畫面進入下一輪並取得一次抽獎資格；抽獎不扣點，`PointRedemptions` 的終身累計點數仍完整保留。
- 管理員可新增或軟刪除轉盤類型，各類型可設定 2～12 個獎項的名稱、色塊與中獎機率。會員在獨立抽獎頁選擇其中一種轉盤，同一個完成輪次只能抽一次。
...
電話與生日會追加為 `Members.phone` 與 `Members.birthday`，原有 1–21 欄位置不會改變。點數功能使用 `PointTypes`、`PointCampaigns`、`PointRedemptions`；集點卡與轉盤功能使用 `PointCardSettings`、`LotteryTypes`、`LotteryPrizes`、`LotteryDraws`，都不會增加 `Members` 欄位。新版 `setup()` 會替舊點數資料補上期限／領取規則快照、建立預設 5 點集點卡與「經典轉盤」，並把既有轉盤獎項歸入該預設類型。既有活動預設遷移為「有期限＋每位會員一次」。升級時請在維護時段連續完成：
...
QR 連結可以被轉傳，因此它代表「持有連結即可參加」而不是現場定位或一次性票券。尤其「無期限＋可重複」QR 外流後，任何仍具會員資格的人都能反覆領點，請只在能接受此風險時使用。若要提前停止活動，可由試算表擁有者把對應 `PointCampaigns.status` 改為 `inactive`。刪除會員資料時也會刪除該會員的 `PointRedemptions` 與 `LotteryDraws`，因此累計點數、集點輪次、領取及抽獎歷史會一併清除；若日後重新建立會員，仍有效的舊 QR 可依活動規則重新領取。

## 集點卡與轉盤抽獎流程

1. 管理員前往 `admin/lottery.html` 設定每輪所需點數，例如 `5`。規則採版本化追加；新規則只計算生效後取得的點數，不改寫舊規則、舊輪次或已取得的抽獎資格。
2. 管理員可新增多個轉盤類型，並替每個類型設定 2～12 個獎項的名稱、`#RRGGBB` 色塊與中獎機率。每個機率最多兩位小數，合計必須精確等於 100%。刪除類型採軟刪除，舊設定與抽獎紀錄仍保留。
3. `PointRedemptions` 的點數永遠代表終身累計。例如門檻為 5，累計取得第 5 點時完成第 1 輪、集點卡顯示從 0 開始第 2 輪，並產生一次可用抽獎資格；累計取得第 10 點時再產生一次。
4. 會員由會員資料頁前往獨立的 `client/lottery.html`，查看本輪進度與可抽次數，再選擇一個轉盤。一次完成輪次只能用於任一類型的一次抽獎，不能在不同轉盤重複使用。
5. 抽獎不扣點。會員 GAS 會在鎖定狀態下重新確認會員權限與未使用輪次，由後台依設定機率決定獎項，再把轉盤類型、設定版本、集點卡規則版本與輪次鍵寫入 `LotteryDraws`。抽獎前後累計點數相同。
6. 前端不自行決定獎項，只依 GAS 回傳結果播放動畫。結果畫面顯示抽獎前與抽獎後累計，按「確認並返回會員資料」才離開。
7. 每次操作會保留固定 request ID。網路逾時後以同一 request ID 重試只會重播原結果，不會重複使用輪次。管理端可查看最新 50 筆抽獎紀錄，但不會收到 LINE User ID 或 request ID。
...
會員 LIFF ID Token
  ...
  → PointCardSettings：依生效版本計算每輪門檻
  → LotteryTypes／LotteryPrizes：讀取可用轉盤類型、目前版本與機率
  → LotteryDraws：後台決定獎項後寫入類型、集點輪次與結果快照，不扣累計點數
