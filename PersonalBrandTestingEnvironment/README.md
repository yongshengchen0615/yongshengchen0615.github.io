...
- 會員登入先完成 `upsertMemberIdentity` 並顯示身分卡，再漸進讀取 `getMemberCardSummary`，避免點數帳本或轉盤資料拖慢首屏。讀取會依每個 GAS URL 記住最近成功的 Fetch／bridge 通道，網路層失敗時最多切換一次；所有會員與管理寫入固定走受驗證的 iframe bridge，逾時時保留同一 request ID 供安全重試，不會改走另一通道造成重複寫入。只儲存通道名稱，不儲存會員資料。
...
本版後端版本是會員 GAS `1.14.0`、管理 GAS `1.11.0`。發布時先更新並部署兩套 GAS，分別確認健康檢查版本，再發布 `shared/`、`client/` 與 `admin/`。會員 GAS 仍提供 `prepareLotteryDraw` 與 `reportClientPerformance` action；不要先發布新版前端，否則舊後端可能回傳 `UNSUPPORTED_ACTION`。
