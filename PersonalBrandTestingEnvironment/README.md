...
- 會員資料頁的「點數紀錄」按鈕會開啟與抽獎券一致的滿版清單，顯示本人最近 30 筆領點與抽獎結果；只有第一次開啟或資料變更時才讀取後台。
...
- 開啟轉盤時只呼叫一次 `prepareLotteryDraw`，後台會同時驗證抽獎券、保存結果並回傳最新轉盤；會員確認票券後，中央按鈕只播放 3 秒由快到慢的動畫，不會在旋轉途中讀取網路。
- 會員登入會先讀取公開健康檢查能力：新版後端使用 `upsertMemberIdentity` 顯示身分卡，再漸進讀取 `getMemberCardSummary`；仍在部署中的舊版後端則自動使用相容的 `upsertMember`，避免前後端發布不同步時才失敗。讀取會依每個 GAS URL 記住最近成功的 Fetch／bridge 通道，網路層失敗時最多切換一次；所有會員與管理寫入固定走受驗證的 iframe bridge。會員登入同步允許最多 45 秒完成冷啟動，其他寫入維持 25 秒；逾時時保留同一 request ID 供安全重試，不會改走另一通道造成重複寫入。只儲存通道名稱，不儲存會員資料。
- 管理端會員搜尋由 GAS 對完整資料集篩選後再分頁；點數與抽獎紀錄以 cursor 逐頁讀取並可匯出 CSV。點數及抽獎編輯草稿只存於當前分頁的 `sessionStorage`，成功送出即清除，離頁前會提醒尚未儲存的修改。
- 會員與管理 GAS 對同一個有效 LINE ID Token 最多快取 5 分鐘的 `sub`、`iat`、`exp` 驗證結果；原始 Token、姓名、頭像與聯絡資料不會進入快取。只有會員端逾時、失敗或超過目標時間的匿名效能事件會送到 Cloud Logging。
...
本版後端版本是會員 GAS `1.15.0`、管理 GAS `1.11.0`。發布時仍建議先更新並部署兩套 GAS，分別確認健康檢查版本，再發布 `shared/`、`client/` 與 `admin/`。會員前端會依健康檢查的 `capabilities` 在新版兩階段登入與舊版 `upsertMember` 之間選擇相容合約；會員 GAS 仍提供 `prepareLotteryDraw` 與 `reportClientPerformance` action。

### 從既有版本升級

電話與生日會追加為 `Members.phone` 與 `Members.birthday`，原有 1–21 欄位置不會改變。點數功能使用 `PointTypes`、`PointCampaigns`、`PointRedemptions`；集點卡與轉盤功能使用 `PointCardSettings`、`LotteryTypes`、`LotteryPrizes`、`LotteryDraws`，都不會增加 `Members` 欄位。新版 `setup()` 會替舊點數資料補上期限／領取規則快照，在 `PointCardSettings` 尾端追加 `reward_milestones`、`reward_lottery_type_ids`、`expiry_mode` 與 `expires_on`，並在 `LotteryTypes` 尾端追加 `show_prizes_on_ticket` 與 `ticket_prize_orders`。舊轉盤的抽獎券獎項預覽預設關閉，不會改變既有會員畫面；先前已開啟整組預覽的轉盤會相容解讀為全部獎項皆勾選。管理員可在完整轉盤設定中逐項選擇要公開的獎項名稱。舊集點卡會安全補成無期限；節點與轉盤欄逐項對應，記錄每個節點唯一可使用的轉盤。全新安裝的 `LotteryTypes` 只有欄位標題，不會自動建立轉盤；請先從管理端按「新增第一個轉盤」完成設定，再替預設 5 點節點選擇該轉盤並儲存集點卡。只有舊版已存在轉盤獎項時，升級程序才會建立「經典轉盤」、把舊獎項歸入該類型並將舊節點指派給它。舊集點卡規則的空白節點相容解讀為「只在滿點抽一次」；既有活動預設遷移為「有期限＋每位會員一次」。升級時請在維護時段連續完成：

1. 同時將最新的會員 `Code.gs` 與管理 `Code.gs` 貼入各自專案。
2. 先在管理 GAS、再在會員 GAS 各執行一次 `setup()`，確認 `Members` 為 23 欄、`LotteryTypes` 為 11 欄、三張點數工作表已完成升級，並已建立 `PointCardSettings`、`LotteryPrizes`、`LotteryDraws`。
...
5. 會員資料頁的「點數紀錄」滿版清單會透過 `listPointHistory` 合併本人最近 30 筆領點與轉盤結果；其他會員的 LINE User ID、Email 與內部欄位不會送到前端。
6. 管理員點數頁會透過 `adminListPointHistory` 先顯示共用 `PointRedemptions` 最近 20 筆領取紀錄，並以 cursor 繼續載入或匯出目前已載入資料；內容包含會員編號、增加點數、領取後累計與領取規則，LINE User ID、請求識別碼等內部欄位不會送到前端。
...
QR 連結可以被轉傳，因此它代表「持有連結即可參加」而不是現場定位或一次性票券。尤其「無期限＋可重複」QR 外流後，任何仍具會員資格的人都能反覆領點，請只在能接受此風險時使用。若要提前停止活動，可由試算表擁有者把對應 `PointCampaigns.status` 改為 `inactive`。會員首頁不提供刪除入口；服務經營者依隱私權政策處理刪除要求時，相關 `PointRedemptions` 與 `LotteryDraws`、累計點數、集點卡進度、領取及抽獎歷史也應一併清除。

## 集點卡與轉盤抽獎流程

1. 初始狀態沒有轉盤。管理員前往 `admin/lottery.html` 按「新增第一個轉盤」，在同一個表單設定名稱、2～12 個獎項、`#RRGGBB` 色塊與中獎機率，並逐項勾選要顯示在抽獎券上的獎項，預覽確認後只按一次「儲存並啟用轉盤」。每個機率最多兩位小數，合計必須精確等於 100%；可不勾選、只勾選一項或勾選多項，抽獎券只顯示所選獎項名稱，不公開機率。
...
3. `PointRedemptions` 的點數永遠代表終身累計，會員卡會把這個數字放在會員名稱旁並顯示為「累計點數」。以上述 20 點卡為例，本張卡到 5、10、15、20 點時各產生一張指定轉盤的抽獎券；到 20 點後會員畫面仍顯示第 2 張卡「本卡進度 0 點／目標 20 點」，累計點數則保留為 20。
4. 會員卡以一點一顆星呈現本輪集點，不使用進度條；可取得抽獎券的節點改用星芒標示。「抽獎券」會開啟滿版清單並切換「未獲得／已獲得」，有勾選公開獎項的轉盤會在兩種票券下方只列出管理員選取的可抽中獎項；「點數紀錄」也使用相同的滿版高度與獨立內容捲動。點選可用券後，會員首頁會立即顯示「正在準備轉盤」，獎項、圖面與本次結果都安全準備完成後才切換為置中的小型轉盤；`client/lottery.html` 僅保留舊連結相容。
5. 抽獎不扣點。會員 GAS 會在鎖定狀態下重新確認會員權限、抽獎券與指定轉盤，由後台依設定機率決定獎項，再把轉盤類型、設定版本、集點卡規則版本、卡片編號與節點寫入 `LotteryDraws`。抽獎前後累計點數相同；完成後該券直接從「已獲得」清單移除。
6. 前端不自行決定獎項。轉盤設定、畫布與 GAS 決定的本次結果都會在載入畫面中準備完成；中央按下後不再發送網路請求，只執行一次由快到慢的連續動畫並平順對準得獎區塊。安全準備結果與旋轉期間，返回抽獎券、關閉與離頁操作都會鎖定；結果確認後關閉轉盤並回到滿版抽獎券清單。
7. 每次操作會依會員保存固定 request ID，展示模式使用獨立空間。網路逾時後以同一 request ID 重試只會重播原結果，不會重複使用節點資格；後台明確確認票券不存在時則解除待確認狀態並重新同步。管理端可查看最新 50 筆抽獎紀錄，但不會收到 LINE User ID 或 request ID。
