...
- 管理員可設定一張集點卡的總點數與多個抽獎節點，例如總點數 `20`、節點 `5, 10, 15, 20`。每到一個節點取得一次抽獎資格；集滿後畫面開始下一張卡，但 `PointRedemptions` 的終身累計點數仍完整保留。
- 管理員可新增或軟刪除轉盤。新增時在同一個編輯區完成名稱、2～12 個獎項、色塊與中獎機率，最後只需按一次「儲存並啟用轉盤」。會員在獨立抽獎頁選擇其中一種轉盤，每個已取得的節點資格只能抽一次。
...
電話與生日會追加為 `Members.phone` 與 `Members.birthday`，原有 1–21 欄位置不會改變。點數功能使用 `PointTypes`、`PointCampaigns`、`PointRedemptions`；集點卡與轉盤功能使用 `PointCardSettings`、`LotteryTypes`、`LotteryPrizes`、`LotteryDraws`，都不會增加 `Members` 欄位。新版 `setup()` 會替舊點數資料補上期限／領取規則快照，並在 `PointCardSettings` 尾端追加 `reward_milestones`、建立預設 5 點集點卡。全新安裝的 `LotteryTypes` 只有欄位標題，不會自動建立轉盤；請從管理端按「新增第一個轉盤」完成設定。只有舊版已存在轉盤獎項時，升級程序才會建立「經典轉盤」並把舊獎項歸入該類型。舊集點卡規則的節點欄會保持空白並相容解讀為「只在滿點抽一次」；既有活動預設遷移為「有期限＋每位會員一次」。升級時請在維護時段連續完成：
...
1. 管理員前往 `admin/lottery.html` 設定一張卡的總點數與抽獎節點，例如總點數 `20`、節點 `5, 10, 15, 20`。節點必須是不重複的遞增整數，最後一個節點必須等於總點數。規則採版本化追加；新規則只計算生效後取得的點數，不改寫舊規則、舊卡片或已取得的抽獎資格。
2. 初始狀態沒有轉盤。管理員按「新增第一個轉盤」後，在同一個表單依序設定轉盤名稱、2～12 個獎項、`#RRGGBB` 色塊與中獎機率，預覽確認後只按一次「儲存並啟用轉盤」即可。每個機率最多兩位小數，合計必須精確等於 100%。既有轉盤也在同一表單一次儲存名稱與獎項；刪除採軟刪除，舊設定與抽獎紀錄仍保留。
3. `PointRedemptions` 的點數永遠代表終身累計。以上述 20 點卡為例，本張卡到 5、10、15、20 點時各產生一次抽獎資格；到 20 點後會員畫面顯示第 2 張卡的 `0 / 20`，但終身累計仍為 20。
4. 會員由會員資料頁前往獨立的 `client/lottery.html`，查看本張卡進度、四個節點與可抽次數，再選擇一個轉盤。每個節點資格只能用於任一類型的一次抽獎，不能在不同轉盤重複使用。
5. 抽獎不扣點。會員 GAS 會在鎖定狀態下重新確認會員權限與最早未使用的節點資格，由後台依設定機率決定獎項，再把轉盤類型、設定版本、集點卡規則版本、卡片編號與節點寫入 `LotteryDraws`。抽獎前後累計點數相同。
...
會員 LIFF ID Token
  ...
  → PointCardSettings：依生效版本計算卡片總點數與抽獎節點
