...
- 已核准管理員可建立 `1 點`～`9999 點`的點數類型，選擇到期時間後產生會員 LIFF QR Code。
- 已核准會員掃描 QR 後會先看到點數與期限，只有按下「確認領取」才會加點；同一活動每位會員只能領取一次。
...
電話與生日會追加為 `Members.phone` 與 `Members.birthday`，原有 1–21 欄位置不會改變。點數功能另外建立 `PointTypes`、`PointCampaigns` 與 `PointRedemptions`，不會再增加 `Members` 欄位。升級時請在維護時段連續完成：

1. 同時將最新的會員 `Code.gs` 與管理 `Code.gs` 貼入各自專案。
2. 在兩邊各執行一次 `setup()`，確認 `Members` 為 23 欄，且三張點數工作表的第一列已建立。
...
1. 管理員在後台新增整數點數類型，例如 `1 點`、`2 點`、`3 點`。
2. 選取一個類型與未來到期時間（最長 366 天），產生 QR Code；可下載 PNG 或複製會員 LIFF 連結。
3. 會員掃描後登入，系統先驗證會員仍為 `approved`、活動為 `active` 且尚未到期。
4. 會員按「確認領取」後，會員 GAS 才把紀錄寫入 `PointRedemptions`，並從領取紀錄加總目前餘額。

QR 只包含一個 43 字元的不透明 claim，不包含點數、會員 ID 或 LINE Token。`PointCampaigns` 只保存 claim 的 SHA-256 hash；點數則使用建立活動當下的後台快照，前端傳入的數值不會被採信。同一個 QR 可以提供多位會員使用，但同一個已驗證會員對同一活動只能成功領取一次。

QR 連結可以被轉傳，因此它代表「持有連結即可參加」而不是現場定位或一次性票券。請設定合理的到期時間；若要提前停止，可由試算表擁有者把對應 `PointCampaigns.status` 改為 `inactive`。刪除會員資料時也會刪除該會員的 `PointRedemptions`，因此點數餘額與領取歷史會一併清除；若日後重新建立會員，尚未到期的舊 QR 將可再次領取。
