...
會員端 LIFF Channel 還必須在 LINE Developers Console 連結會員官方帳號，且該 LIFF 頁面要從官方帳號的一對一聊天室開啟，`sendMessages()` 才會把訊息送到正確的官方帳號。若會員尚未加入官方帳號，系統會先顯示「加入官方帳號」按鈕，按下後開啟 `client/config.json` 的 `OFFICIAL_ACCOUNT_FRIEND_URL`；未完成加入就不會寫入點數。從一般瀏覽器或外部 QR 頁面開啟時也使用相同按鈕流程。
