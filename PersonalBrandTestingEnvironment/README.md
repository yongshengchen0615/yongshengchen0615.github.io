...
- 已核准會員掃描 QR 並登入後會直接領取點數；結果畫面會顯示原本、獲得與目前點數。從集點卡頁掃描時，按「確認」會更新並返回集點卡；活動可設定每會員一次、重新掃描後重複領取，或整張 QR 僅開放第一位會員。
- 集點卡頁會顯示最近 30 筆點數紀錄，包含獲得點數、抽獎結果、發生時間、QR 規則與當時累計點數，並提供重新整理、載入、空狀態與錯誤回饋。
- 管理員可逐筆新增一張集點卡的抽獎節點，例如總點數 `20`，分別建立 `5`、`10`、`15`、`20` 點節點；每個節點必須指定且只能指定一個已完成設定的轉盤。集滿後畫面開始下一張卡，但 `PointRedemptions` 的終身累計點數仍完整保留。
- 管理員可新增或軟刪除轉盤。新增時在同一個編輯區完成名稱、2～12 個獎項、色塊與中獎機率，最後只需按一次「儲存並啟用轉盤」。會員在獨立集點卡頁分別查看「未獲得」節點與「已獲得」可用券；轉盤資料會在畫面開啟前預載，中央一按便立即旋轉並同步等待 GAS 結果。抽完的券會直接從已獲得清單移除。
- 會員不需要加入官方帳號即可註冊與集點；「掃描集點 QR Code」已整合到集點卡頁。若頁面是在官方帳號一對一聊天室中開啟，且 LIFF 具備 `chat_message.write` 權限，首次建立會員與領點成功後會額外嘗試用 `sendMessages()` 傳送通知，通知失敗不影響會員建立或點數發放。
...
├── client/                    # 會員端 LIFF
│   ├── index.html
│   ├── lottery.html           # 集點卡、掃碼領點、點數紀錄、抽獎券分頁與轉盤
│   ├── lottery.js
│   ├── privacy.html
│   ├── styles.css
│   ├── script.js
...
會員端 LIFF App 應在 LINE Developers Console 開啟 `Scan QR` 功能，集點卡頁的「掃描集點 QR Code」按鈕會優先呼叫 `liff.scanCodeV2()`。建議 Size 選 `Full`，並在實機上授予相機權限。若 LIFF 原生掃描因 `subwindowOpen` 或環境限制不可用，前端會在支援 `BarcodeDetector` 與 `getUserMedia` 的瀏覽器自動切換成頁內後鏡頭掃描；若兩種能力都不可用，才顯示設定提示。從管理端產生的點數 QR 也可以直接開啟會員 LIFF，保留原本的自動領點流程。
...
3. 會員可在集點卡頁按「掃描集點 QR Code」使用 LIFF 相機掃描，也可以直接開啟 QR 連結；系統會先驗證會員仍為 `approved`、活動為 `active`，有期限活動還會核對是否到期。
4. 會員掃描並完成登入後，會員 GAS 直接把領取紀錄寫入 `PointRedemptions`；結果畫面顯示原本點數、此次獲得點數與目前點數。從集點卡頁掃描時，按「確認」會重新載入集點卡進度。
5. 集點卡頁會透過 `listPointHistory` 合併本人最近 30 筆領點與轉盤結果紀錄；其他會員的 LINE User ID、Email 與內部欄位不會送到前端。
...
4. 會員由會員資料頁的「集點卡」前往 `client/lottery.html`，先看到集點進度、掃碼按鈕、點數紀錄與抽獎券。抽獎券以「未獲得／已獲得」分頁切換；所有轉盤設定與圖面會在開啟轉盤前預載，點選可用券後才切換到獨立轉盤畫面，會員點轉盤中央開始抽獎，不能自行改選其他轉盤。
...
6. 前端不自行決定獎項。轉盤設定與畫布會先預載，中央按下後立即開始旋轉，同時等待 GAS 安全決定結果，再平順對準得獎區塊。結果畫面顯示抽獎前與抽獎後累計，按「確認並返回集點卡」後仍停留在集點卡頁。
