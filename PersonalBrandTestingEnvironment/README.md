...
4. 會員卡直接顯示目前集點與期限；「抽獎券」會開啟滿版清單並切換「未獲得／已獲得」，「點數紀錄」維持小視窗。點選可用券後，會員首頁會立即顯示「正在準備轉盤」，設定與圖面完成後才切換為置中的小型轉盤；`client/lottery.html` 僅保留舊連結相容。
...
6. 前端不自行決定獎項。轉盤設定與畫布會在載入畫面中準備完成，中央按下後立即旋轉，同時等待 GAS 安全決定結果，再由快到慢平順對準得獎區塊。旋轉中返回抽獎券、關閉與離頁操作都會鎖定；結果確認後關閉轉盤並回到滿版抽獎券清單。
