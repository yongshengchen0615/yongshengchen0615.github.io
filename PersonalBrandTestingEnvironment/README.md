...
- 管理員可新增或軟刪除轉盤。新增時在同一個編輯區完成名稱、2～12 個獎項、色塊與中獎機率，最後只需按一次「儲存並啟用轉盤」。會員按「抽獎券」會直接在會員資料頁的小視窗切換「未獲得／已獲得」，只有選定可用券時才開啟轉盤頁。轉盤由快到慢平順停下，開獎期間所有返回導覽都會暫停，抽完的券會直接移除。
...
├── client/                    # 會員端 LIFF
│   ├── index.html
│   ├── lottery.html           # 選定抽獎券後的轉盤與舊連結相容畫面
│   ├── lottery.js
│   ├── privacy.html
│   ├── styles.css
│   ├── script.js
...
4. 會員卡直接顯示目前集點與期限；「抽獎券」及「點數紀錄」都在會員資料頁以小視窗開啟。抽獎券可切換「未獲得／已獲得」，點選可用券才前往 `client/lottery.html`；轉盤設定與圖面會在中央按鈕可操作前完成載入。
...
6. 前端不自行決定獎項。轉盤設定與畫布會先預載，中央按下後立即旋轉，同時等待 GAS 安全決定結果，再由快到慢平順對準得獎區塊。旋轉中返回抽獎券、會員資料與離頁操作都會鎖定；結果確認後回到會員資料頁並重新開啟抽獎券小視窗。
