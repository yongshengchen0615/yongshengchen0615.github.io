(() => {
  ...
  window.addEventListener('DOMContentLoaded', () => {
    [
      'app', 'loadingView', 'loadingProgress', 'loadingProgressBar', 'errorView', 'errorTitle', 'errorMessage', 'pendingBox', 'pendingUserId', 'retryButton', 'adminView', 'displayName', 'roleLabel', 'logoutButton',
  ...
  function bindEvents() {
    ...
    els.addCalendarBatchItemButton.addEventListener('click', addCalendarBatchItem);
  ...
  function renderAll() {
    ...
    renderMembers(); renderTierSettings(); renderCardList(); renderTicketList(); renderEventTicketList(); renderCalendarItemList();
  ...
  function fieldLabel(labelText, type, value, options) {
    ...
    if (type === 'select') { input = document.createElement('select'); (options.options || []).forEach(([optionValue, optionLabel]) => { const option = document.createElement('option'); option.value = optionValue; option.textContent = optionLabel; input.append(option); }); input.value = String(value || ''); input.title = String(input.selectedOptions[0] && input.selectedOptions[0].textContent || ''); input.addEventListener('change', () => { input.title = String(input.selectedOptions[0] && input.selectedOptions[0].textContent || ''); }); } else { input = document.createElement('input'); input.type = type; input.value = value === undefined || value === null ? '' : String(value); if (options.min) input.min = options.min; if (options.max) input.max = options.max; if (options.step) input.step = options.step; if (options.maxlength) input.maxLength = Number(options.maxlength); if (options.placeholder) input.placeholder = options.placeholder; }
  ...
  async function saveCard(event) {
    ...
    const validationMessage = validatePublicStatus(els.cardStatus.value, '集點卡') || validateRewardEditor(String(els.cardTitle.value || '').trim(), rewards) || validateCardExpiry(expiryMode, expiresOn);
    if (validationMessage) return showMessage(els.cardFormMessage, validationMessage);
    setSaving(els.saveCardButton, true, '正在儲存集點卡…');
    try {
      const result = await window.MemberSystem.request(state.config, 'admin', state.idToken, 'admin.pointcards.save', { card: { cardId: els.cardId.value, title: String(els.cardTitle.value || '').trim(), description: String(els.cardDescription.value || '').trim(), rewardTitle: '', rewards, status: els.cardStatus.value, styleKey: safeTierStyle(els.cardStyle.value), expiryMode, expiresOn, accent: safeAccent(els.cardAccent.value) }, expectedUpdatedAt: els.cardExpectedUpdatedAt.value });
  ...
  function renderCalendarItemList() {
    els.calendarItemResultCount.textContent = String(state.calendarItems.length);
    els.calendarItemEmptyState.classList.toggle('hidden', state.calendarItems.length !== 0);
    els.calendarItemListItems.replaceChildren(...state.calendarItems.map((item) => {
      const row = document.createElement('div'); row.className = 'calendar-list-row';
      const selectLabel = document.createElement('label'); selectLabel.className = 'calendar-item-selector';
      const select = document.createElement('input'); select.type = 'checkbox'; select.dataset.calendarItemSelect = String(item.calendarItemId || ''); select.checked = state.selectedCalendarItemIds.has(String(item.calendarItemId || ''));
      select.setAttribute('aria-label', `選取${String(item.title || '日曆項目')}進行批次處理`);
  ...
  function handleCalendarItemSelectionChange(event) {
    ...
    if (!input || !input.dataset.calendarItemSelect) return;
    const calendarItemId = String(input.dataset.calendarItemSelect || '');
  ...
  function addCalendarBatchItem() {
    synchronizeCalendarBatchItemsFromDom();
    if (state.calendarBatchItems.length >= 20) return showMessage(els.calendarBatchMessage, '每次最多可批次處理 20 筆日曆項目。');
    state.calendarBatchItems.push(calendarBatchItemFromCalendarItem(null));
  ...
  function renderCalendarBatchControls() {
    ...
    els.addCalendarBatchItemButton.disabled = batchCount >= 20 || state.writeConfirmationRequired;
    ...
    els.calendarBatchSummary.textContent = batchCount ? `待儲存 ${batchCount} 筆${selectedCount ? `；另選取 ${selectedCount} 筆可批次刪除或加入修改` : ''}。` : selectedCount ? `已選取 ${selectedCount} 筆，可加入批次修改或批次刪除。` : '尚未加入批次項目。';
  }
  ...
  async function saveCalendarBatch() {
    ...
    } catch (error) { handleActionError(error, els.calendarBatchMessage); } finally { setSaving(els.saveCalendarBatchButton, false); renderCalendarBatchRows(); }
  }
