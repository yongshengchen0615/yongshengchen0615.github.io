(() => {
  ...
  function collectEventTicketAllowedTiers() { return Array.from(new FormData(els.eventTicketForm).getAll('eventTicketAllowedTierKey')).map((value) => String(value || '').trim()).filter((tierKey) => EVENT_TICKET_TIER_KEYS.includes(tierKey)); }
