(() => {
  ...
  function bindEvents() {
    ...
    els.rewardRows.addEventListener('click', (event) => {
      ...
      if (addPrize) { const row = addPrize.closest('[data-reward-row]'); renderLotteryPrizes(row, [...collectPrizes(row), defaultPrize()]); updateRewardEditorHint(); return; }
  ...
  function createRewardRow(reward, index) {
    ...
    const prizeHeading = document.createElement('div'); prizeHeading.className = 'prize-heading'; const prizeLabel = document.createElement('strong'); prizeLabel.textContent = '抽獎獎項'; const totalLabel = document.createElement('span'); totalLabel.dataset.prizeTotal = 'true'; prizeHeading.append(prizeLabel, totalLabel); const addPrize = document.createElement('button'); addPrize.type = 'button'; addPrize.className = 'text-button'; addPrize.dataset.addPrize = 'true'; addPrize.textContent = '＋ 新增獎項'; prizeHeading.append(addPrize);
  ...
  function renderLotteryPrizes(row, prizes) { const prizeRows = row.querySelector('[data-prize-rows]'); prizeRows.replaceChildren(...prizes.map((prize, index) => createPrizeRow(prize, index))); updatePrizeTotal(row); }
  function createPrizeRow(prize, index) { const row = document.createElement('div'); row.className = 'prize-row'; row.dataset.prizeRow = 'true'; row.append(fieldLabel(`獎項 ${index + 1}`, 'text', prize.prizeTitle, { field: 'prizeTitle', maxlength: '100', placeholder: '例如：頭獎／免費蛋糕' }), fieldLabel('機率', 'number', prize.winRate, { field: 'winRate', min: '0', max: '100', step: '0.01', suffix: '%' })); const description = fieldLabel('獎項說明（選填）', 'text', prize.prizeDescription, { field: 'prizeDescription', maxlength: '240', placeholder: '例如：可兌換任一蛋糕' }); description.classList.add('prize-description-field'); row.append(description); const remove = document.createElement('button'); remove.type = 'button'; remove.className = 'text-button remove-prize'; remove.dataset.removePrize = 'true'; remove.setAttribute('aria-label', `刪除獎項 ${index + 1}`); remove.textContent = '刪除'; row.append(remove); return row; }
  ...
  function handleRewardRowsInput(event) { const row = event.target.closest('[data-reward-row]'); if (!row) return; if (event.target.dataset.field === 'rewardType') { const prizeEditor = row.querySelector('[data-lottery-prizes]'); const lottery = event.target.value === 'lottery'; prizeEditor.classList.toggle('hidden', !lottery); if (lottery && !row.querySelectorAll('[data-prize-row]').length) renderLotteryPrizes(row, [defaultPrize()]); } updatePrizeTotal(row); updateRewardEditorHint(); }
  function updatePrizeTotal(row) { const prizeRows = row.querySelectorAll('[data-prize-row]'); const total = Array.from(prizeRows).reduce((sum, prizeRow) => sum + Math.round(Number(prizeRow.querySelector('[data-field="winRate"]')?.value || 0) * 100), 0); const totalLabel = row.querySelector('[data-prize-total]'); if (totalLabel) { totalLabel.textContent = `獎項機率合計 ${formatRate(total / 100)}%${total === 10000 ? ' ✓' : '（需為 100%）'}`; totalLabel.classList.toggle('warning', total !== 10000); } }
  function updateRewardEditorHint() { const rewards = collectRewards(); const target = Number(els.cardTargetStamps.value); const duplicate = rewards.some((reward, index) => rewards.findIndex((item) => item.thresholdStamps === reward.thresholdStamps) !== index); const incompleteLottery = rewards.some((reward) => reward.rewardType === 'lottery' && Math.round(reward.prizes.reduce((sum, prize) => sum + Number(prize.winRate || 0), 0) * 100) !== 10000); els.rewardEditorHint.textContent = duplicate ? '有節點使用相同點數，請調整後再儲存。' : incompleteLottery ? '抽獎券的獎項機率合計必須正好是 100%。0% 獎項仍可保留。' : rewards.length ? `${rewards.length} 個節點 · 最後一個節點為 ${Math.max(...rewards.map((reward) => reward.thresholdStamps || 0)) || '—'} 點${Number.isInteger(target) ? `／完成點數 ${target} 點` : ''}` : '請至少新增 1 個節點。'; els.rewardEditorHint.classList.toggle('warning', duplicate || incompleteLottery); }
