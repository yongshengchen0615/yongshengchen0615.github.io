(() => {
  ...
  // Remove the former display-cache database without reading it. Every screen
  // now receives its bootstrap payload directly from GAS after authentication.
  function removeLegacyDisplayCache() {
    try { if (window.indexedDB && typeof window.indexedDB.deleteDatabase === 'function') window.indexedDB.deleteDatabase('MembershipSystemSyncCache'); } catch (_) {}
  }
  removeLegacyDisplayCache();
