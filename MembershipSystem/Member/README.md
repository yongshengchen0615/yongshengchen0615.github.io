...
日曆沿用同一個管理端，但會員端是獨立的 calendar HTML、CSS 與 JavaScript。管理員可新增、修改或刪除兩種類型的日期：休假日與活動；每筆可設定單日或最長 366 天的跨日區間、說明、識別色與公開狀態。管理端可一次新增或修改最多 20 筆，亦可勾選多筆既有日期後批次刪除；伺服器會先驗證整批資料與版本，再開始寫入。只有啟用中的日期會顯示在會員日曆；開啟時只讀取上月、本月、下月三個完整月份，切換時不再發出讀取請求，超出範圍會直接提示。會員可用月曆按鈕切換；手機也可水平滑動、桌面則可在月曆上使用滑鼠滾輪。會員點擊有項目的日期時，會在小視窗中逐一查看該日所有相關說明；彈窗不顯示休假日或活動類型標記。日曆與會員卡、集點卡、活動票券共用相同的會員階級進度區塊，只顯示目前階級、累積服務分鐘、下一階段門檻、所需分鐘與進度條。
