(() => {
  ...
  function renderMilestones(card, stamps) {
    ...
    els.milestoneList.replaceChildren(...rewards.map((reward) => {
      ...
      const copy = document.createElement('div'); const title = document.createElement('strong'); title.textContent = String(reward.rewardTitle || '節點回饋'); const meta = document.createElement('small'); meta.textContent = `${rewardTypeLabel(reward.rewardType)} · ${stamps >= Number(reward.thresholdStamps || 0) ? '已達成' : `還差 ${Math.max(0, Number(reward.thresholdStamps || 0) - stamps)} 點`}`; copy.append(title, meta); if (reward.rewardDescription) { const description = document.createElement('p'); description.textContent = String(reward.rewardDescription); copy.append(description); } if (String(reward.rewardType) === 'lottery') { const prizeLabel = document.createElement('p'); prizeLabel.className = 'prize-opportunity-label'; prizeLabel.textContent = '有機會獲得'; copy.append(prizeLabel); const prizeList = document.createElement('ul'); prizeList.className = 'prize-opportunities'; (Array.isArray(reward.prizes) ? reward.prizes : []).forEach((prize) => { const prizeItem = document.createElement('li'); prizeItem.className = 'prize-opportunity'; const prizeName = document.createElement('span'); prizeName.textContent = String(prize.prizeTitle || '未命名獎項'); const prizeRate = document.createElement('span'); prizeRate.textContent = `${formatRate(prize.winRate)}%`; prizeItem.append(prizeName, prizeRate); prizeList.append(prizeItem); }); if (prizeList.children.length) copy.append(prizeList); } item.append(marker, copy); return item;
    }));
  }
