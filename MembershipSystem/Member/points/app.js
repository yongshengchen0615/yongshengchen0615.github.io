(() => {
  ...
  function renderActiveCard(card) {
    ...
    els.activeCardStatus.textContent = card.status === 'archived' ? '已停止' : complete ? '已達成' : '進行中';
    ...
    els.progressMessage.textContent = card.status === 'archived' ? '這張卡已停止集點，已取得的票券仍可使用' : complete ? '恭喜，已達成兌換條件' : stamps ? '保持這個節奏，繼續累積' : '開始累積你的第一點';
    els.remainingMessage.textContent = card.status === 'archived' ? '請使用已取得的票券' : complete ? '可以向店家兌換' : `還差 ${target - stamps} 點`;
  ...
  function renderMilestones(card, stamps) {
    ...
    els.milestoneList.replaceChildren(...rewards.map((reward) => {
      const reached = stamps >= Number(reward.thresholdStamps || 0);
      const ticket = state.tickets.find((item) => item.cardId === card.cardId && item.thresholdStamps === Number(reward.thresholdStamps || 0));
      const item = document.createElement('article'); item.className = `milestone-item${reached ? ' reached' : ''}`;
      const marker = document.createElement('span'); marker.className = 'milestone-marker'; marker.textContent = reached ? '✓' : String(reward.thresholdStamps || '—');
      const copy = document.createElement('div'); const title = document.createElement('strong'); title.textContent = String(reward.rewardTitle || '節點回饋'); const meta = document.createElement('small'); meta.textContent = `${rewardTypeLabel(reward.rewardType)} · ${reached ? '已達成' : `還差 ${Math.max(0, Number(reward.thresholdStamps || 0) - stamps)} 點`}`; copy.append(title, meta); if (reward.rewardDescription) { const description = document.createElement('p'); description.textContent = String(reward.rewardDescription); copy.append(description); } if (String(reward.rewardType) === 'lottery') { const prizeLabel = document.createElement('p'); prizeLabel.className = 'prize-opportunity-label'; prizeLabel.textContent = '可能獲得'; copy.append(prizeLabel); const prizeList = document.createElement('ul'); prizeList.className = 'prize-opportunities'; (Array.isArray(reward.prizes) ? reward.prizes : []).forEach((prize) => { const prizeItem = document.createElement('li'); prizeItem.className = 'prize-opportunity'; const prizeName = document.createElement('span'); prizeName.textContent = String(prize.prizeTitle || '未命名獎項'); prizeItem.append(prizeName); prizeList.append(prizeItem); }); if (prizeList.children.length) copy.append(prizeList); }
      const action = document.createElement('div'); action.className = 'milestone-action'; if (reached && ticket) { if (ticket.status === 'used') { const used = document.createElement('span'); used.className = 'ticket-used'; used.textContent = '已使用'; action.append(used); } else if (ticket.status === 'locked') { const locked = document.createElement('span'); locked.className = 'ticket-used'; locked.textContent = '已鎖定'; action.append(locked); } else { const useButton = document.createElement('button'); useButton.type = 'button'; useButton.className = 'small-ticket-button'; useButton.dataset.useTicket = ticket.ticketId; useButton.textContent = '使用這張券'; action.append(useButton); } } item.append(marker, copy, action); return item;
    }));
  }
  ...
  function replaceTicket(tickets, next) { return tickets.map((ticket) => ticket.ticketId === next.ticketId ? next : ticket); }
