...
test('browser clients use ID tokens and do not persist credentials', () => {
  ...
  assert.match(clients, /signIn\(state\.config, 'calendar'\)/);
...
test('CSP allows the LIFF subwindow without unsafe-eval', () => {
  const html = [read('member/index.html'), read('points/index.html'), read('event/index.html'), read('calendar/index.html'), read('admin/index.html')].join('\n');
...
test('ID tokens are not explicitly written to logs or Sheets', () => {
  const gas = ['gas/Code.gs', 'gas/Auth.gs', 'gas/Storage.gs', 'gas/MemberService.gs', 'gas/PointCardService.gs', 'gas/EventTicketService.gs', 'gas/CalendarService.gs'].map(read).join('\n');
