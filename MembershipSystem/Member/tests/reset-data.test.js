...
test('manual new-environment reset clears every data table and restores only default tier settings', () => {
  const cache = new Map();
  const sheetNames = ['Members', 'Admins', 'PointCards', 'PointCardRewards', 'PointCardLotteryPrizes', 'PointCardTicketTemplates', 'PointCardTickets', 'PointCardTicketChallenges', 'EventTickets', 'EventTicketClaims', 'CalendarItems', 'PointBalances', 'PointEntries', 'PointMutations', 'ServiceTimeEntries', 'LineNotificationLogs', 'MembershipTierSettings', 'AuditLogs'];
