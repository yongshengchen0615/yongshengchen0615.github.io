(() => {
  ...
  function showConfigNotice() {
    ...
    if (!el) return;
    el.textContent = '尚未設定 GAS Web App URL。請先依 README 部署 GAS，並更新 config.json。';
  ...
  function renderUser(data) {
    ...
    const balance = Number(data.card.balance);
  ...
  async function loadUser() {
    renderUser(await state.bridge.call('getMyCard', { sessionToken: state.sessionToken }));
  }
  ...
  function bindEvents() {
    ...
    $('refreshBtn')?.addEventListener('click', () => loadUser().catch((error) => showNotice(error.message, 'error')));
  ...
  async function init() {
    ...
    try {
      ...
      if (/未授權|工作階段|登入/.test(error.message)) state.sessionToken = '';
      showNotice(error.message, 'error');
    }
  }
