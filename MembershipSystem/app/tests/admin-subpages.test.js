...
test('admin management is split into five focused subpages', () => {
  for (const page of ['overview', 'members', 'grants', 'tiers', 'usage']) {
  ...
  assert.match(adminHtml, /id="adminPageGrants"/);
...
test('existing management functions stay inside their dedicated subpages', () => {
  const membersPage = adminHtml.match(/<section id="adminPageMembers"[\s\S]*?<\/section>\s*<section id="adminPageGrants"/)[0];
  const grantsPage = adminHtml.match(/<section id="adminPageGrants"[\s\S]*?<\/section>\s*<section id="adminPageTiers"/)[0];
  ...
  assert.match(grantsPage, /id="minuteGrantMemberSearch"/);
  assert.match(grantsPage, /id="minuteGrantReason"/);
...
test('subpage navigation supports direct hash navigation and overview shortcuts', () => {
  ...
  assert.match(adminHtml, /data-admin-page-target="grants"/);
  assert.match(adminHtml, /data-admin-page-target="tiers"/);
  assert.match(adminHtml, /data-admin-page-target="usage"/);
  assert.match(subpagesScript, /PAGE_NAMES = \['overview', 'members', 'grants', 'tiers', 'usage'\]/);
