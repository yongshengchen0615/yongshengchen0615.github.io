...
- `我的開團`：篩選自己建立的團
- `我已加入`：篩選自己有下單的團
- `開新團`：建立團名、開始/結束下單時間、品項、金額、品項細項與加價選項，也可先儲存為草稿
...
- 品項細項：團主可在開新團或品項管理先新增共用的品項細項與加價選項，並設定每組細項是否下單必填；下單時可針對同一品項新增不同細項明細
- 即時同步：登入後會自動同步開團列表與目前明細；正在下單、修改訂單或使用團主儲存相關表單時，會暫停覆蓋畫面，等操作完成後再更新
