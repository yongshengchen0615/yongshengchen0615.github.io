(() => {
  ...
  function setupModal() {
    ...
    modal.innerHTML = `<div class="booking-admin-modal-card"><div class="booking-admin-modal-heading"><h2 id="bookingCrudModalTitle">管理</h2><button id="bookingCrudModalClose" class="booking-admin-modal-close" type="button">×</button></div><div id="bookingCrudModalBody"></div></div>`;
  ...
  function renderTypes() {
    ...
    state.data.serviceTypes.forEach((type) => {
      ...
      const edit = button('修改', 'button button-outline', () => openTypeModal(type));
      const remove = button('刪除', 'button button-danger', () => deleteType(type));
  ...
  function openServiceModal(service = null) {
    ...
    form.addEventListener('submit', async (event) => {
      event.preventDefault();
      const payload = readServiceForm(form);
  ...
  function openBatchModal(mode) {
    ...
    const rows = els.modalBody.querySelector('#bookingCrudBatchRows');
  ...
  function appendBatchRow(container, service, mode) {
    ...
    row.innerHTML = serviceFields(service);
    row.querySelector('[data-field="serviceType"]').value = service.serviceType || '';
    if (mode === 'create') {
      const remove = button('移除此列', 'button button-outline', () => row.remove());
  ...
  async function runModalAction(fn) {
    ...
    state.busy = true;
    const message = els.modalBody.querySelector('#bookingCrudModalMessage');
    try { await fn(); closeModal(); await refresh(); document.getElementById('bookingAdminRefreshButton')?.click(); }
  ...
  async function runPageAction(fn) {
    if (state.busy) return;
    state.busy = true; clearMessage(els.serviceMessage);
