...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    catch { return reply(origin, { ok:false,status:400,error:{ code:"INVALID_JSON",message:"請求內容必須是有效 JSON。" } }, 400); }
    ...
    try { verifiedPayload = JSON.parse(verifiedText); }
    catch { return reply(origin, { ok:false,status:503,error:{ code:"UPSTREAM_INVALID_RESPONSE",message:"會員驗證服務暫時無法正常回應。" } }, 503); }
    if (!verifiedResponse.ok || verifiedPayload?.ok !== true) {
      return reply(origin, verifiedPayload, Number(verifiedPayload?.status || verifiedResponse.status || 500));
    }

    const profile = verifiedPayload?.data?.profile && typeof verifiedPayload.data.profile === "object"
      ? verifiedPayload.data.profile
      : null;
    ...
    if (action === "user.calendar.date.details") {
      ...
      rows = rows.filter((row:any) => String(row.starts_on) <= date && String(row.ends_on || row.starts_on) >= date);
    }

    return reply(origin, { ok:true,status:200,data:{ profile,items:rows.map(calendarClient) } }, 200);
  } catch {
    return reply(origin, { ok:false,status:503,error:{ code:"CALENDAR_SERVICE_UNAVAILABLE",message:"會員日曆服務暫時無法使用。" } }, 503);
  }
});
