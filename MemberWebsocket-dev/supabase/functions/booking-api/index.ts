...
function mapDatabaseError(error: unknown): ApiError {
  ...
  const rules: Array<[string, number, string, string]> = [
    ["BOOKING_SLOT_TAKEN", 409, "BOOKING_SLOT_TAKEN", "這個時段剛剛已被其他會員預約，請選擇其他時間。"],
    ["BOOKING_TOO_EARLY", 409, "BOOKING_TOO_EARLY", "尚未符合提前預約天數，請選擇較晚的日期。"],
    ["BOOKING_TIME_PASSED", 409, "BOOKING_TIME_PASSED", "這個預約時間已經過了，請重新選擇。"],
    ["BOOKING_DAY_UNAVAILABLE", 409, "BOOKING_DAY_UNAVAILABLE", "這一天沒有開放此預約項目。"],
    ["BOOKING_SERVICE_DISABLED", 409, "BOOKING_SERVICE_DISABLED", "這個預約項目目前未開放。"],
    ["BOOKING_SERVICE_NOT_FOUND", 404, "BOOKING_SERVICE_NOT_FOUND", "找不到這個預約項目。"],
    ["INVALID_BOOKING_SLOT", 400, "INVALID_BOOKING_SLOT", "預約時段不在目前開放的工作時間內。"],
    ["REQUEST_ID_CONFLICT", 409, "REQUEST_ID_CONFLICT", "操作識別碼衝突，請重新操作。"],
    ["MEMBERSHIP_REQUIRED", 403, "MEMBERSHIP_REQUIRED", "請先完成會員加入後再使用預約功能。"],
    ["MEMBER_DISABLED", 403, "MEMBER_DISABLED", "此會員目前已停用，無法預約。"],
    ["MEMBER_NOT_FOUND", 404, "MEMBER_NOT_FOUND", "找不到會員資料，請先完成會員加入。"],
  ];
  for (const [needle, status, code, userMessage] of rules) {
    if (message.includes(needle)) return new ApiError(status, code, userMessage);
  }
  if (raw?.code === "23505") return new ApiError(409, "BOOKING_SLOT_TAKEN", "這個時段已被預約，請選擇其他時間。");
...
async function requireJoinedMember(supabase: SupabaseClient, identity: Identity): Promise<any> {
  ...
  if (!member) throw new ApiError(404, "MEMBER_NOT_FOUND", "找不到會員資料，請先完成會員加入。");
  if (member.membership_status !== "active") throw new ApiError(403, "MEMBERSHIP_REQUIRED", "請先完成會員加入後再使用預約功能。");
...
function normalizeTime(value: unknown): string {
  ...
  if (hour < 0 || hour > 23 || ![0, 30].includes(minute)) throw new ApiError(400, "INVALID_TIME", "時間必須以 30 分鐘為單位。");
...
function serviceClient(row: any): Json {
  return {
    ...
    workStartTime: String(row.work_start_time || "").slice(0, 5),
    workEndTime: String(row.work_end_time || "").slice(0, 5),
    slotMinutes: Number(row.slot_minutes || SLOT_MINUTES),
    minAdvanceDays: Number(row.min_advance_days || 0),
    availableWeekdays: Array.isArray(row.available_weekdays) ? row.available_weekdays.map(Number) : [0,1,2,3,4,5,6],
...
function bookingClient(row: any): Json {
  const service = row.booking_services || row.service || null;
  const member = row.members || row.member || null;
  return {
    bookingId: row.id,
    requestId: row.request_id,
    serviceId: row.service_id,
    serviceTitle: service?.title || row.service_title || "預約項目",
...
async function generateSlots(supabase: SupabaseClient, serviceId: string, date: string): Promise<{ service: Json; slots: Json[]; earliestBookingDate: string }> {
  ...
  if (!service) throw new ApiError(404, "BOOKING_SERVICE_NOT_FOUND", "找不到這個預約項目。");
  if (!service.is_active) throw new ApiError(409, "BOOKING_SERVICE_DISABLED", "這個預約項目目前未開放。");

  const today = taipeiDate();
  const earliestBookingDate = addDays(today, Number(service.min_advance_days || 0));
  const weekdays = Array.isArray(service.available_weekdays) ? service.available_weekdays.map(Number) : [0,1,2,3,4,5,6];
  if (date < earliestBookingDate || !weekdays.includes(weekdayFor(date))) {
    return { service: serviceClient(service), slots: [], earliestBookingDate };
  }
  ...
  const occupied = new Set((bookings.data || []).map((row: any) => String(row.start_time || "").slice(0, 5)));

  const start = timeToMinutes(String(service.work_start_time).slice(0, 5));
  const end = timeToMinutes(String(service.work_end_time).slice(0, 5));
  const nowMinutes = taipeiMinutes();
  const slots: Json[] = [];
  for (let cursor = start; cursor + SLOT_MINUTES <= end; cursor += SLOT_MINUTES) {
    const startTime = minutesToTime(cursor);
    const available = !occupied.has(startTime) && !(date === today && cursor <= nowMinutes);
    slots.push({ startTime, endTime: minutesToTime(cursor + SLOT_MINUTES), available });
  }
  return { service: serviceClient(service), slots, earliestBookingDate };
}
...
async function userBootstrap(supabase: SupabaseClient, member: any): Promise<Json> {
  const [services, bookings] = await Promise.all([
    ...
    supabase.from("bookings")
      .select("*, booking_services(title)")
      .eq("member_id", member.id)
      .order("booking_date", { ascending: false })
      .order("start_time", { ascending: false })
      .limit(50),
  ]);
  if ((bookings as any).error) throw mapDatabaseError((bookings as any).error);
  const today = taipeiDate();
  return {
    today,
    services: services.map(serviceClient),
    bookings: (((bookings as any).data || []) as any[]).map(bookingClient),
  };
}
...
async function userCreate(supabase: SupabaseClient, identity: Identity, member: any, body: Json): Promise<Json> {
  ...
  const created = await supabase.rpc("create_booking_request", {
  ...
  const serviceResult = await supabase.from("booking_services").select("title").eq("id", row.service_id).single();
  const mapped = bookingClient({ ...row, booking_services: serviceResult.data || null });
  await audit(supabase, identity, "member", "BOOKING_REQUESTED", "booking", String(row.id), {
    serviceId, bookingDate, startTime,
  });
  return { booking: mapped };
}
...
async function userCancel(supabase: SupabaseClient, identity: Identity, member: any, body: Json): Promise<Json> {
  ...
  }).eq("id", bookingId).eq("member_id", member.id).in("status", ["pending", "confirmed"]).select("*, booking_services(title)").single();
  ...
  return { booking: bookingClient(updated.data) };
}

function normalizeWeekdays(value: unknown): number[] {
  if (!Array.isArray(value)) throw new ApiError(400, "INVALID_WEEKDAYS", "請至少選擇一個可預約星期。");
  const days = [...new Set(value.map(Number).filter((day) => Number.isInteger(day) && day >= 0 && day <= 6))].sort((a, b) => a - b);
  if (!days.length) throw new ApiError(400, "INVALID_WEEKDAYS", "請至少選擇一個可預約星期。");
  return days;
}
...
async function adminServiceSave(supabase: SupabaseClient, identity: Identity, body: Json): Promise<Json> {
  ...
  if (timeToMinutes(workEndTime) - timeToMinutes(workStartTime) < SLOT_MINUTES) {
    throw new ApiError(400, "INVALID_WORK_HOURS", "結束工作時間必須晚於開始時間至少 30 分鐘。");
  }
  ...
  const availableWeekdays = normalizeWeekdays(body.availableWeekdays);
  ...
  if (serviceId) {
    ...
    const current = await supabase.from("booking_services").select("updated_at").eq("id", serviceId).maybeSingle();
    ...
    if (expectedUpdatedAt && current.data.updated_at !== expectedUpdatedAt) {
...
async function adminBookings(supabase: SupabaseClient): Promise<Json[]> {
  ...
  const pending = await supabase.from("bookings")
    .select("*, booking_services(title), members(display_name, member_code)")
    .eq("status", "pending")
...
async function adminBootstrap(supabase: SupabaseClient): Promise<Json> {
  const servicesResult = await supabase.from("booking_services").select("*").order("created_at", { ascending: true });
  if (servicesResult.error) throw mapDatabaseError(servicesResult.error);
  return {
    today: taipeiDate(),
    services: (servicesResult.data || []).map(serviceClient),
    bookings: await adminBookings(supabase),
  };
}
...
async function adminStatusUpdate(supabase: SupabaseClient, identity: Identity, body: Json): Promise<Json> {
  ...
  const updated = await supabase.from("bookings").update(patch).eq("id", bookingId).eq("status", booking.status)
    .select("*, booking_services(title), members(display_name, member_code)").single();
  if (updated.error) throw mapDatabaseError(updated.error);
...
async function route(supabase: SupabaseClient, identity: Identity, clientType: ClientType, action: string, body: Json): Promise<Json> {
  if (clientType === "member") {
    ...
    if (action === "user.booking.slots") {
      const serviceId = requireUuid(body.serviceId, "預約項目");
      const bookingDate = requireDate(body.bookingDate);
      return await generateSlots(supabase, serviceId, bookingDate);
    }
