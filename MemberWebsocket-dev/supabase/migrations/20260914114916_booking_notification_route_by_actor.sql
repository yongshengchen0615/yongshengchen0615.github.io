...
begin
  if TG_OP = 'INSERT' then
    ...
    -- A booking UPDATE is not automatically a user-visible modification.
    -- Creation/contact synchronization and profile synchronization can update
    -- the row internally. Only explicit booking-edit RPCs write one of these
    -- audit events in the same transaction (same now() timestamp).
    select e.id, e.actor_role, e.action
      into mod_audit_id, mod_actor_role, mod_action
  ...
  -- For state-transition events, ensure this deferred trigger still represents
  -- the final state at commit time. Modification events may intentionally move
  -- confirmed -> pending, so they are validated by their audit event instead.
  ...
  -- Member-side actions notify admins only.
  ...
  -- Admin-side actions notify the member only.
