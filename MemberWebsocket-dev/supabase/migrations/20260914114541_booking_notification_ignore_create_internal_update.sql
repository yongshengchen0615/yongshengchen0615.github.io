...
begin
  if TG_OP='INSERT' then
    ...
    and NEW.updated_at > NEW.created_at then
    -- Booking creation performs an internal UPDATE in the same transaction.
    -- touch_booking_updated_at() uses the transaction timestamp, so a create-time
    -- internal UPDATE keeps updated_at = created_at and must not be reported as
    -- a user/admin modification.
    event_kind := 'modified';
