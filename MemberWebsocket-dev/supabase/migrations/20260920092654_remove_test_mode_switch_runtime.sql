...
begin
  ...
  if not coalesce(p_maintenance_enabled, false) then
    ...
    if not coalesce(p_allow_pc_test_login, false) then
      update public.test_login_sessions
      set revoked_at = coalesce(revoked_at, now())
      where revoked_at is null and device_class = 'pc';
    end if;
    if not coalesce(p_allow_mobile_test_login, false) then
      update public.test_login_sessions
      set revoked_at = coalesce(revoked_at, now())
      where revoked_at is null and device_class = 'mobile';
    end if;
  end if;
...
comment on column public.test_mode_settings.enabled is
  'Deprecated compatibility flag. Runtime access is controlled by maintenance_enabled plus device-specific test login flags.';
