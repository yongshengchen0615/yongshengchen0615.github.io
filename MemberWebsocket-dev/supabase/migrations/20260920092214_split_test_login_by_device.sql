alter table public.test_mode_settings
  add column if not exists allow_pc_test_login boolean not null default false,
  add column if not exists allow_mobile_test_login boolean not null default false;

update public.test_mode_settings
set allow_pc_test_login = true,
    allow_mobile_test_login = false
where id = true
  and allow_pc_test_login = false
  and allow_mobile_test_login = false;

comment on column public.test_mode_settings.allow_pc_test_login is
  'Allow desktop/PC user surfaces to enter direct test-account login while system maintenance and test mode are both enabled.';
comment on column public.test_mode_settings.allow_mobile_test_login is
  'Allow mobile/tablet user surfaces to enter direct test-account login while system maintenance and test mode are both enabled.';

create or replace function public.admin_save_test_mode_v3(
...
begin
  ...
  insert into public.test_mode_settings (
    ...
    allow_pc_test_login, allow_mobile_test_login,
...
revoke all on function public.admin_save_test_mode_v3(boolean, boolean, boolean, boolean, text, text, integer)
  from public, anon, authenticated;
grant execute on function public.admin_save_test_mode_v3(boolean, boolean, boolean, boolean, text, text, integer)
  to service_role;
