...
begin
  ...
  if not found then
  ...
  elsif v_existing_primary_technician_id is null
     or not exists (
       select 1 from public.booking_technicians t
        where t.id = v_existing_primary_technician_id
    ...
    update public.booking_settings
     ...
     where id = 1;
  elsif (select max_party_size from public.booking_settings where id=1) < 2 then
    update public.booking_settings
