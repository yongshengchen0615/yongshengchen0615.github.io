-- Keep booking LINE notification transport enabled across one-click public data resets.
-- Notification delivery configuration is operational configuration, not test/business data.
-- Preserve booking_notifications.config while still clearing the outbox.
...
begin
  ...
  where schemaname in ('public','booking_notifications')
    ...
    and not (schemaname = 'booking_notifications' and tablename = 'config');
  ...
  truncate table public.realtime_events restart identity;
