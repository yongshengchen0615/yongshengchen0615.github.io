...
begin
  if not exists (
    ...
    where conrelid = 'public.test_login_sessions'::regclass
      and conname = 'test_login_sessions_surface_check'
  ) then
    alter table public.test_login_sessions
      add constraint test_login_sessions_surface_check
      check (surface is null or surface = any (array['member','points','event','calendar','booking']));
  end if;
end
$$;

create index if not exists test_login_sessions_member_surface_idx
  on public.test_login_sessions(member_id, surface, revoked_at, expires_at);

CREATE OR REPLACE FUNCTION public.create_test_login_session_v2(p_token_hash text, p_member_id uuid, p_surface text, p_device_class text, p_expires_at timestamp with time zone)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  perform pg_advisory_xact_lock(hashtextextended(p_member_id::text || ':' || p_surface, 0));
  ...
  if exists (
    select 1
      from public.member_presence_sessions ps
     where ps.member_id = p_member_id
       and ps.surface = p_surface
       and ps.offline_at is null
       and ps.last_seen_at >= v_now - interval '90 seconds'
  ) or exists (
    select 1
      from public.test_login_sessions s
     where s.member_id = p_member_id
       and s.surface = p_surface
       and s.revoked_at is null
       and s.expires_at > v_now
       and s.created_at >= v_now - interval '15 seconds'
  ) then
  ...
  insert into public.test_login_sessions(
    token_hash, member_id, device_class, surface, expires_at
  )
  values (
    p_token_hash, p_member_id, p_device_class, p_surface, p_expires_at
  )
...
revoke all on function public.create_test_login_session_v2(text,uuid,text,text,timestamptz) from public, anon, authenticated;
grant execute on function public.create_test_login_session_v2(text,uuid,text,text,timestamptz) to service_role;

CREATE OR REPLACE FUNCTION public.notify_test_presence_realtime_change()
 RETURNS trigger
 LANGUAGE plpgsql
...
begin
  if tg_op = 'UPDATE' and old.offline_at is not distinct from new.offline_at then
    return null;
  end if;
  ...
  if exists (
    ...
    perform public.emit_realtime_invalidation(array['all'], 'test_mode.presence.changed');
  end if;
  return null;
end
$function$
...
drop trigger if exists test_presence_realtime_after_change on public.member_presence_sessions;
create trigger test_presence_realtime_after_change
after insert or update of offline_at on public.member_presence_sessions
for each row execute function public.notify_test_presence_realtime_change();
...
revoke all on function public.admin_prepare_complex_e2e_fixtures(text,text) from public, anon, authenticated;
grant execute on function public.admin_prepare_complex_e2e_fixtures(text,text) to service_role;
...
begin
  ...
  return jsonb_build_object(
    ...
    'deletedExtendedQaArtifacts', v_fixed + v_services + v_types + v_technicians
  );
end
$function$
;
