-- Birthday fixed tickets are an annual entitlement.
-- Ticket validity remains the member's birthday month, but changing birthday later
-- in the same calendar year must never create a second birthday entitlement.

update public.fixed_ticket_grants g
set cycle_key = 'birthday:' || extract(year from g.cycle_start)::integer::text,
    updated_at = now()
from public.fixed_ticket_templates t
where t.id = g.fixed_ticket_template_id
  and t.schedule_type = 'birthday_month'
  and g.cycle_key <> 'birthday:' || extract(year from g.cycle_start)::integer::text;
